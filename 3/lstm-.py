...
def generate_corrosion_data(num_pipes=300, time_steps=100):
    """
    生成模拟的管道腐蚀数据集（含时间序列）
    参数:
        num_pipes: 管道数量
    ...
    for pipe_id in tqdm(range(num_pipes), desc="生成管道数据"):
        ...
        # 腐蚀积累效应（LSTM特别适合建模这种长期依赖）
        corrosion_accumulation = np.zeros(time_steps)
        for t in range(1, time_steps):
            # 腐蚀积累取决于历史腐蚀速率和环境因素
            corrosion_accumulation[t] = (
                    0.7 * corrosion_accumulation[t - 1] +
                    0.2 * temp_effect[t] +
                    0.1 * cl_effect[t]
            )
        ...
        data['corrosion_rate'][start_idx:end_idx] = (
                ...
                material_factor * coating_factor + 0.5 * corrosion_accumulation +
...
def explore_data(df):
    ...
    env_features = ['temperature', 'Cl_conc', 'flow_rate', 'pH']
    for i, feature in enumerate(env_features):
        ax = axes[i // 2, i % 2]
        sns.scatterplot(data=df, x=feature, y='corrosion_rate', alpha=0.2, ax=ax)
        ax.set_title(f'腐蚀速率 vs {feature}')
        ax.set_ylabel('腐蚀速率 (mm/year)')
    plt.tight_layout()
    plt.savefig('env_vs_corrosion.png', dpi=300)
    plt.show()
...
def feature_engineering(df):
    ...
    # 滞后特征（前几个时间步的腐蚀速率）
    for lag in [1, 2, 3, 5]:
        df[f'corrosion_rate_lag{lag}'] = df.groupby('pipe_id')['corrosion_rate'].shift(lag)

    # 移动平均特征
    for window in [3, 5, 7]:
        df[f'corrosion_rate_ma{window}'] = df.groupby('pipe_id')['corrosion_rate'].transform(
            lambda x: x.rolling(window=window, min_periods=1).mean()
        )

    # 指数加权移动平均
    df['corrosion_rate_ewm'] = df.groupby('pipe_id')['corrosion_rate'].transform(
...
# 4. 数据预处理
def preprocess_data(df, time_steps=15):
    """
    预处理数据：分割、转换、缩放
    参数:
    ...
    numerical_features = [
        ...
        'corrosion_rate_lag5', 'corrosion_rate_ma3', 'corrosion_rate_ma5',
        'corrosion_rate_ma7', 'corrosion_rate_ewm'
    ]
    ...
    def create_sequences(data, targets, pipe_ids, seq_length):
        ...
        for pipe_id in tqdm(np.unique(pipe_ids), desc="创建时间序列"):
...
# 5. 构建LSTM模型
def build_lstm_model(input_shape, lstm_units=128, dense_units=64, dropout_rate=0.3, learning_rate=0.001):
    """
    构建用于腐蚀预测的LSTM模型
    参数:
    ...
    返回:
        编译好的Keras模型
    """
    model = Sequential(name="PipeCorrosionLSTMPredictor")
    ...
    # 全连接层
    model.add(Dense(dense_units, activation='relu', kernel_regularizer=l2(1e-4), name="dense1"))
    model.add(Dropout(dropout_rate * 0.8, name="dropout3"))
    model.add(Dense(dense_units // 2, activation='relu', kernel_regularizer=l2(1e-4), name="dense2"))
...
def train_model(model, X_train, y_train, X_val, y_val):
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            filepath='best_corrosion_lstm_model.h5',
            monitor='val_loss',
            save_best_only=True,
...
# 8. 时间序列预测可视化
def visualize_time_series_predictions(model, df, preprocessor, pipe_id, time_steps=15):
    """
    可视化特定管道的腐蚀速率预测
    参数:
        model: 训练好的模型
    ...
    for i in range(len(X_preprocessed) - time_steps):
        ...
        actual_values.append(pipe_data['corrosion_rate'].iloc[i + time_steps])
    ...
    print(pipe_data['time_step'].index)
    # 标记关键点
    max_corrosion_idx = np.argmax(pipe_data['corrosion_rate'])
    plt.scatter(
        pipe_data['time_step'].iloc[max_corrosion_idx],
        pipe_data['corrosion_rate'].iloc[max_corrosion_idx],
        color='purple', s=100, zorder=5,
        label=f'最大腐蚀点 ({pipe_data["corrosion_rate"].iloc[max_corrosion_idx]:.4f} mm/year)'
    )
...
class CorrosionPredictor:
    ...
    def __init__(self, model_path='best_corrosion_lstm_model.h5',
...
def main():
    ...
    print("=" * 50)
    print("管道腐蚀预测模型 - LSTM实现")
    print("=" * 50)

    # 步骤1: 生成模拟数据
    print("\n步骤1: 生成模拟管道腐蚀数据（含时间序列）...")
    corrosion_data = generate_corrosion_data(num_pipes=300, time_steps=100)
    ...
    print("衍生特征已添加: temp_cl_index, flow_pressure_ratio, corrosion_aggressiveness, 滞后特征, 移动平均等")
    ...
    print("\n步骤7: 评估模型性能...")
    # 加载最佳模型
    best_model = load_model('best_corrosion_lstm_model.h5')
    performance = evaluate_model(best_model, X_test, y_test, history)

    # 保存性能报告
    pd.Series(performance).to_csv('model_performance.csv')
    print("模型性能已保存到 'model_performance.csv'")

    # 步骤8: 可视化时间序列预测
    print("\n步骤8: 可视化时间序列预测...")

    # 随机选择几个管道进行可视化
    sample_pipes = np.random.choice(corrosion_data['pipe_id'].unique(), 3, replace=False)
    for pipe_id in sample_pipes:
        print(f"可视化管道 {pipe_id} 的预测...")
        predictions, actuals = visualize_time_series_predictions(
            best_model, corrosion_data, preprocessor, pipe_id, time_steps
        )

        # 计算该管道的预测性能
        pipe_mae = mean_absolute_error(actuals, predictions)
        pipe_rmse = np.sqrt(mean_squared_error(actuals, predictions))
        print(f"管道 {pipe_id} 预测性能: MAE={pipe_mae:.6f}, RMSE={pipe_rmse:.6f}")

    # 步骤9: 保存预处理器和模型
    print("\n步骤9: 保存模型和预处理器...")
    joblib.dump(preprocessor, 'corrosion_preprocessor.pkl')
    best_model.save('corrosion_lstm_model_final.h5')
    print("模型和预处理器已保存")

    # ---------- 步骤 10：多步滚动预测示例 ----------
    print("\n步骤10: 多步滚动预测示例...")

    # 重新实例化 predictor（确保路径正确）
    predictor = CorrosionPredictor(
        model_path='best_corrosion_lstm_model.h5',
        preprocessor_path='corrosion_preprocessor.pkl',
        time_steps=time_steps
    )

    # 选一条管道做示例
    demo_pipe_id = sample_pipes[0]
    pipe_full = corrosion_data[corrosion_data['pipe_id'] == demo_pipe_id].copy()

    n_future = 10
    history = pipe_full.tail(time_steps).copy().reset_index(drop=True)

    future_ts, future_pred = [], []

    for k in range(1, n_future + 1):
        # 预测下一步
        pred_rate = predictor.predict(history)

        # 构造下一行特征
        new_row = history.iloc[-1:].copy()

        # 数值特征：上一值 + 小扰动
        for col in ['temperature', 'pressure', 'flow_rate', 'Cl_conc',
                    'pH', 'O2_content', 'CO2_conc', 'H2S_conc']:
            new_row[col] += np.random.normal(0, 0.02 * abs(new_row[col].values[0]))

        # 类别特征保持不变
        new_row['material'] = new_row['material'].values[0]
        new_row['coating'] = new_row['coating'].values[0]

        # 写入时间步和预测值
        new_row['time_step'] = history['time_step'].max() + 1
        new_row['corrosion_rate'] = pred_rate
        new_row['pipe_id'] = demo_pipe_id

        # 重新衍生特征
        new_row = feature_engineering(new_row)

        # 追加并截断到 time_steps
        history = pd.concat([history, new_row], ignore_index=True)
        if len(history) > time_steps:
            history = history.iloc[-time_steps:].reset_index(drop=True)

        future_ts.append(new_row['time_step'].values[0])
        future_pred.append(pred_rate)

    # 打印结果
    print(f"\n管道 {demo_pipe_id} 未来 {n_future} 个时间步预测结果:")
    for t, p in zip(future_ts, future_pred):
        print(f"time_step={t}, 预测腐蚀速率={p:.6f} mm/year")
'''
    print(f"\n示例预测结果 (管道 {pipe_id}):")
    print(f"时间步 {next_time_step} 的预测腐蚀速率: {prediction:.6f} mm/year")
    print(f"时间步 {next_time_step} 的实际腐蚀速率: {actual_value:.6f} mm/year")
